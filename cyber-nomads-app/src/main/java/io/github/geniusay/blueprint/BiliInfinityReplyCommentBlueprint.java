...
import io.github.geniusay.core.supertask.plugin.comment.AIReplyCommentGenerate;
...
public class BiliInfinityReplyCommentBlueprint extends AbstractTaskBlueprint {
    ...
    protected void executeTask(RobotWorker robot, Task task) throws Exception {
        ...
        String content = taskPluginFactory.<AbstractCommentGenerate>buildPluginWithGroup(COMMENT_GROUP_NAME, task).generateComment(handleComment.getMessage());
        ApiResponse<Boolean> response = new ActionFlow<>(
                new BiliUserActor(robot),
                new BiliCommentLogic(content),
                new BiliCommentReceiver(handleComment)
        ).execute();
        task.addLastWord(robot, response, Map.of("bvid", handleComment.getBvid(), "username", handleComment.getUsername(), "content", content));
    }
    ...
    protected String lastWord(RobotWorker robot, Task task, LastWord lastWord) {
        ...
        String comment = (String) lastWord.getAdditionalInfo("content");
